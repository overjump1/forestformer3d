...
def isolate_trees(pcd):
    """Run the full three-stage treeiso segmentation on an above-ground cloud.

    Args:
        pcd: (N, 3) array of x/y/z coordinates. Ground points must already be
            removed (treeiso assumes an above-ground vegetation cloud).

    Returns:
        (final_labels, init_labels, intermediate_labels), each a length-N int
        array. ``final_labels`` are the per-point individual-tree instance ids.

    Note:
        Extracted from the original ``main`` loop so the segmentation can be
        reused programmatically. The intermediate-stage labels are computed on
        the coarser ``PR_DECIMATE_RES2`` grid and are therefore mapped back with
        ``dec_inverse_idx2`` (the RES2 inverse index); the upstream ``main``
        mistakenly used the RES1 inverse index here, which is out of bounds.
    """
    pcd = np.asarray(pcd, dtype=np.float64)[:, :3]
    pcd = pcd - np.mean(pcd, axis=0)

    dec_idx_uidx, dec_inverse_idx = decimate_pcd(pcd, PR_DECIMATE_RES1)  # reduce points first
    pcd_dec = pcd[dec_idx_uidx]

    # Initial segmentation in 3D
    init_labels = init_segs(pcd_dec)

    dec_idx_uidx2, dec_inverse_idx2 = decimate_pcd(pcd, PR_DECIMATE_RES2)  # reduce points first
    pcd_dec2 = pcd[dec_idx_uidx2]

    # Intermediate segmentation in 2D (on the RES2 grid)
    intermediate_labels = intermediate_segs(np.concatenate(
        [pcd_dec2[:, :3], init_labels[dec_inverse_idx][dec_idx_uidx2][:, np.newaxis]], axis=-1))

    # Bring the RES2 intermediate labels onto the RES1-decimated points.
    intermediate_on_dec1 = intermediate_labels[dec_inverse_idx2][dec_idx_uidx]

    # Final segmentation based on segment similarity
    final_labels = final_segs(np.concatenate(
        [pcd_dec, init_labels[:, np.newaxis], intermediate_on_dec1[:, np.newaxis]], axis=-1))

    return (final_labels[dec_inverse_idx],
            init_labels[dec_inverse_idx],
            intermediate_labels[dec_inverse_idx2])
